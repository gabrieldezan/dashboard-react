...
/**
 * Classe BannersMobile - Responsável por manipular banners_mobile no banco de dados.
 */
class BannersMobile
{
    /**
    ...
    /**
     * Lista todos os registros mobile, ordenados pela posição.
     * @return PDOStatement
     */
    public function ler()
    {
        $query = "SELECT * FROM banners_mobile ORDER BY posicao ASC";
        $stmt = $this->conn->prepare($query);
        $stmt->execute();
    ...
    {
        $query = "SELECT * FROM banners_mobile WHERE id = ? LIMIT 1";
        $stmt = $this->conn->prepare($query);
    ...
    {
        $query = "DELETE FROM banners_mobile WHERE id = ?";
        $stmt = $this->conn->prepare($query);
