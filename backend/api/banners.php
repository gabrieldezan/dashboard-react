...
switch ($method) {
    ...
    case 'GET':
        ...
        if (isset($_GET['id'])) {
            $banners->id = $_GET['id'];
            $banner_item = $banners->lerPorId();

            if ($banner_item) {
                http_response_code(200);
                echo json_encode($banner_item);
            } else {
                http_response_code(404);
                echo json_encode(["message" => "Banner não encontrado."]);
            }
        } else {
            $stmt = $banners->ler();
            $banners_arr = [];
    ...
    case 'POST':
        ...
        if (!empty($_POST['id']) && is_numeric($_POST['id'])) {
            ...
            $imagem_atual = $banner_item['imagem'] ?? null;
            $webp_atual = $banner_item['imagem_webp'] ?? null;
            $imagem_largura = $banner_item['imagem_largura'] ?? null;
            $imagem_altura = $banner_item['imagem_altura'] ?? null;
            $thumb_atual = $banner_item['thumb'] ?? null;
            $thumb_largura = $banner_item['thumb_largura'] ?? null;
            $thumb_altura = $banner_item['thumb_altura'] ?? null;
        } else {
            $imagem_atual = "";
            $webp_atual = "";
            $imagem_largura = "";
        ...
        $banners->descricao = $_POST['descricao'];
        $banners->link = $_POST['link'];
        $banners->texto_botao = $_POST['texto_botao'];
        $banners->imagem = $imagem;
        $banners->imagem_webp = $imagem_webp;
        $banners->imagem_largura = $imagem_largura;
        $banners->imagem_altura = $imagem_altura;
        $banners->thumb = $thumb;
        $banners->thumb_largura = $thumb_largura;
        $banners->thumb_altura = $thumb_altura;
        $banners->abrir_nova_guia = filter_var($_POST['abrir_nova_guia'], FILTER_VALIDATE_BOOLEAN) ? 1 : 0;
        $banners->posicao = (int) $_POST['posicao'];
        $banners->status = filter_var($_POST['status'], FILTER_VALIDATE_BOOLEAN) ? 1 : 0;

        // Atualiza ou cria novo
        if (!empty($_POST['id']) && is_numeric($_POST['id'])) {
            if ($banners->atualizar()) {
            ...
            if ($banners->criar()) {
                http_response_code(201);
                echo json_encode(["message" => "Banner criado com sucesso."]);
            } else {
                http_response_code(503);
                echo json_encode(["message" => "Não foi possível criar o banner."]);
            }
        }
    ...
    case 'DELETE':
        ...
        if ($banners->deletar()) {
            http_response_code(200);
            echo json_encode(["message" => "Banner deletado com sucesso."]);
        } else {
            http_response_code(503);
            echo json_encode(["message" => "Não foi possível deletar o banner."]);
        }
